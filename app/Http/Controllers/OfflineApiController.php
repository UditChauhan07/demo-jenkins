...
class OfflineApiController extends Controller
{
    ...
    public function offlineapi(Request $request)
    {
        ...
        if ($user_profile_pic == NULL) {
            $profile_pic = "http://54.82.61.29/public/img/default-profile-img.png";
        } else {
            $profile_pic = "http://54.82.61.29/public/profile_pic/" . $user_profile_pic;
        }
        ...
        return response()->json([
            ...
            'userdetail' => array('userid' => $userid, 'fullname' => $user->name, 'dob' => $user->dob, 'email'=>$user->email, 'gender' => $gender, 'occupation' => $occupation, 'age' => $userage, 'profile_pic' => $profile_pic, 'joining_date' => date_format($loginuserjoinyear, 'Y-m-d'), 'namecompatibilitypercentage' => $namecompatibilitypercentage),
