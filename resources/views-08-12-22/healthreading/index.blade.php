                ...
                <div class="tab-pane" id="chaldean" role="tabpanel">
                    <div class="card-block table-border-style">
                        <table class="table table-striped">
                            ...
                            @foreach ($chaldhealth as $key => $modules)
                            <tr>
                                <td>{{ $modules->number }}</td>
                                <td>{!! $modules->description !!}</td>
                                <td>
                                    <a class="btn btn-info" href="{{ route('healthreading.show',$modules->id) }}"><i class="icon-copy ion-eye"></i></a>
                                    @can('chaldname-edit')   
                                    <a class="btn btn-primary" href="{{ route('healthreading.edit',$modules->id) }}"><i class="icon-copy ti-pencil-alt"></i></a>
                                    @endcan
                                    @can('chaldname-delete')
                                    {!! Form::open(['method' => 'DELETE','route' => ['healthreading.destroy', $modules->id],'style'=>'display:inline', 'onclick' => '']) !!}
