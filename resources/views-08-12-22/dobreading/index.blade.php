                ...
                <div class="tab-pane active" id="pythagorean" role="tabpanel">
                    <div class="card-block table-border-style">
                        <table class="table table-striped">
                            ...
                            @foreach ($phythadob as $key => $module)
                            <tr>
                                <td>{{ $module->number }}</td>
                                <td>{!! $module->description  = Str::of($module->description)->limit(90);!!}</td>
                                <td>
                                    <a class="btn btn-info" href="{{ route('dobreading.show',$module->id) }}"><i class="icon-copy ion-eye"></i></a>
                                    @can('phythadob-edit')
                                    <a class="btn btn-primary" href="{{ route('dobreading.edit',$module->id) }}"><i class="icon-copy ti-pencil-alt"></i></a>
                                    @endcan
                                    @if($module->is_active == 1)
                                    @can('phythadob-delete')   
                                    {!! Form::open(['method' => 'DELETE','route' => ['dobreading.destroy', $module->id ],'style'=>'display:inline', 'onclick' => '']) !!}
                                    ...
                                    @can('phythadob-delete')   
                                    {!! Form::open(['method' => 'post','route' => ['dobreading.unblock', $module->id ],'style'=>'display:inline', 'onclick' => '']) !!}
                ...
                <div class="tab-pane" id="chaldean" role="tabpanel">
                    <div class="card-block table-border-style">
                        <table class="table table-striped">
                            ...
                            @foreach ($chalddob as $key => $modules)
                            <tr>
                                <td>{{ $modules->number }}</td>
                                <td>{!! $modules->description = Str::of($modules->description)->limit(90); !!}</td>
                                <td>
                                    <a class="btn btn-info" href="{{ route('dobreading.show',$modules->id) }}"><i class="icon-copy ion-eye"></i></a>
                                    @can('chalddob-edit')   
                                    <a class="btn btn-primary" href="{{ route('dobreading.edit',$modules->id) }}"><i class="icon-copy ti-pencil-alt"></i></a>
                                    @endcan
                                    @if($module->is_active == 1)
                                    @can('chalddob-delete')   
                                    {!! Form::open(['method' => 'DELETE','route' => ['dobreading.destroy', $module->id ],'style'=>'display:inline', 'onclick' => '']) !!}
