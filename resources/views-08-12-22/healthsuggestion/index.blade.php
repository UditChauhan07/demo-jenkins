                ...
                <div class="tab-pane" id="chaldean" role="tabpanel">
                    <div class="card-block table-border-style">
                        <table class="table table-striped">
                            ...
                            @foreach ($chaldhealthsugg as $key => $modules)
                            <tr>
                                <td>{{ $modules->number }}</td>
                                <td>{!! $modules->description !!}</td>
                                <td>
                                    <a class="btn btn-info" href="{{ route('healthsuggestion.show',$modules->id) }}"><i class="icon-copy ion-eye"></i></a>
                                    @can('suggmonth-edit')   
                                    <a class="btn btn-primary" href="{{ route('healthsuggestion.edit',$modules->id) }}"><i class="icon-copy ti-pencil-alt"></i></a>
                                    @endcan
                                    @if($modules->is_active == 1)
                                    @can('suggmonth-delete')   
                                    {!! Form::open(['method' => 'DELETE','route' => ['healthsuggestion.destroy', $modules->id ],'style'=>'display:inline', 'onclick' => '']) !!}
                                            <button type="submit" name="Delete" class="btn btn-success alert-block"><i class="icon-copy fi-unlock"></i></button>
                                    {!! Form::close() !!}
                                    @endcan
                                    @endif
                                    ...
                                    @can('suggmonth-delete')   
                                    {!! Form::open(['method' => 'post','route' => ['suggestion.unblock', $modules->id ],'style'=>'display:inline', 'onclick' => '']) !!}
                                            <button type="submit" name="Delete" class="btn btn-danger alert-unblock"><i class="icon-copy fi-lock"></i></button>
                                    {!! Form::close() !!}
                                    @endcan
                                    @endif
