                ...
                <div class="tab-pane active" id="pythagorean" role="tabpanel">
                    <div class="card-block table-border-style">
                        <table class="table table-striped">
                            ...
                            @foreach ($phythadestiny as $key => $module)
                            <tr>
                                <td>{{ $module->number }}</td>
                                <td>{!! $module->description = Str::of($module->description)->limit(90); !!}</td>
                                <td>
                                    <a class="btn btn-info" href="{{ route('destinyno.show',$module->id) }}"><i class="icon-copy ion-eye"></i></a>
                                    @can('destinyno-edit')
                                    <a class="btn btn-primary" href="{{ route('destinyno.edit',$module->id) }}"><i class="icon-copy ti-pencil-alt"></i></a>
                                    @endcan
                                    @if($module->is_active == 1)
                                    @can('destinyno-delete')   
                                    {!! Form::open(['method' => 'DELETE','route' => ['destinyno.destroy', $module->id ],'style'=>'display:inline', 'onclick' => '']) !!}
                                    ...
                                    @can('destinyno-delete')   
                                    {!! Form::open(['method' => 'post','route' => ['destinyno.unblock', $module->id ],'style'=>'display:inline', 'onclick' => '']) !!}
                ...
                <div class="tab-pane" id="chaldean" role="tabpanel">
                    <div class="card-block table-border-style">
                        <table class="table table-striped">
                            ...
                            @foreach ($chalddestiny as $key => $modules)
                            <tr>
                                <td>{{ $loop->iteration }}</td>
                                <td>{{ $modules->number }}</td>
                                <td>{{ $modules->description }}</td>
                                <td>
                                    <a class="btn btn-info" href="{{ route('destinyno.show',$modules->id) }}"><i class="icon-copy ion-eye"></i></a>
                                       
                                    <a class="btn btn-primary" href="{{ route('destinyno.edit',$modules->id) }}"><i class="icon-copy ti-pencil-alt"></i></a>
                                    @if($modules->is_active == 1)
                                    @can('destinyno-delete')   
                                    {!! Form::open(['method' => 'DELETE','route' => ['destinyno.destroy', $modules->id ],'style'=>'display:inline', 'onclick' => '']) !!}
                                            <button type="submit" name="Delete" class="btn btn-success alert-block"><i class="icon-copy fi-unlock"></i></button>
                                    {!! Form::close() !!}
                                    @endcan
                                    @endif
                                    ...
                                    @can('destinyno-delete')   
                                    {!! Form::open(['method' => 'post','route' => ['destinyno.unblock', $modules->id ],'style'=>'display:inline', 'onclick' => '']) !!}
                                            <button type="submit" name="Delete" class="btn btn-danger alert-unblock"><i class="icon-copy fi-lock"></i></button>
                                    {!! Form::close() !!}
                                    @endcan
                                    @endif
