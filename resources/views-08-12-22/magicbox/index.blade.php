                ...
                <div class="tab-pane active" id="pythagorean" role="tabpanel">
                    <div class="card-block table-border-style">
                        <table class="table table-striped">
                            ...
                            @foreach ($phythamagic as $key => $module)
                            <tr>
                                <td>{{ $module->number }}</td>
                                <td>{!! $module->description = Str::of($module->description)->limit(90) !!}</td>
                                <td>
                                    <a class="btn btn-info" href="{{ route('magicbox.show',$module->id) }}"><i class="icon-copy ion-eye"></i></a>
                                    @can('magicbox-edit')
                                    <a class="btn btn-primary" href="{{ route('magicbox.edit',$module->id) }}"><i class="icon-copy ti-pencil-alt"></i></a>
                                    @endcan
                                    @if($module->is_active == 1)
                                    @can('magicbox-delete')   
                                    {!! Form::open(['method' => 'DELETE','route' => ['magicbox.destroy', $module->id ],'style'=>'display:inline', 'onclick' => '']) !!}
                                    ...
                                    @can('magicbox-delete')   
                                    {!! Form::open(['method' => 'post','route' => ['magicbox.unblock', $module->id ],'style'=>'display:inline', 'onclick' => '']) !!}
                ...
                <div class="tab-pane" id="chaldean" role="tabpanel">
                    <div class="card-block table-border-style">
                        <table class="table table-striped">
                            ...
                            @foreach ($chaldmagic as $key => $modules)
                            <tr>
                                <td>{{ $modules->number }}</td>
                                <td>{!! $modules->description !!}</td>
                                <td>
                                    <a class="btn btn-info" href="{{ route('magicbox.show',$modules->id) }}"><i class="icon-copy ion-eye"></i></a>
                                    @can('magicbox-edit')   
                                    <a class="btn btn-primary" href="{{ route('magicbox.edit',$modules->id) }}"><i class="icon-copy ti-pencil-alt"></i></a>
                                    @endcan
                                    @if($modules->is_active == 1)
                                    @can('magicbox-delete')   
                                    {!! Form::open(['method' => 'DELETE','route' => ['magicbox.destroy', $modules->id ],'style'=>'display:inline', 'onclick' => '']) !!}
                                            <button type="submit" name="Delete" class="btn btn-success alert-block"><i class="icon-copy fi-unlock"></i></button>
                                    {!! Form::close() !!}
                                    @endcan
                                    @endif
                                    ...
                                    @can('magicbox-delete')   
                                    {!! Form::open(['method' => 'post','route' => ['magicbox.unblock', $modules->id ],'style'=>'display:inline', 'onclick' => '']) !!}
                                            <button type="submit" name="Delete" class="btn btn-danger alert-unblock"><i class="icon-copy fi-lock"></i></button>
                                    {!! Form::close() !!}
                                    @endcan
                                    @endif
