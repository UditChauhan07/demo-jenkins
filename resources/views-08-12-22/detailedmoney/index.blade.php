                ...
                <div class="tab-pane active" id="pythagorean" role="tabpanel">
                    <div class="card-block table-border-style">
                        <table class="table table-striped">
                            ...
                            @foreach ($phythadetailed as $key => $module)
                            <tr>
                                <td>{{ $module->number }}</td>
                                <td>{!! $module->description = Str::of($module->description)->limit(90); !!}</td>
                                <td>
                                    <a class="btn btn-info" href="{{ route('detailedmoney.show',$module->id) }}"><i class="icon-copy ion-eye"></i></a>
                                    @can('detailmoney-edit')
                                    <a class="btn btn-primary" href="{{ route('detailedmoney.edit',$module->id) }}"><i class="icon-copy ti-pencil-alt"></i></a>
                                    @endcan
                                    @if($module->is_active == 1)
                                    @can('detailmoney-delete')   
                                    {!! Form::open(['method' => 'DELETE','route' => ['detailedmoney.destroy', $module->id ],'style'=>'display:inline', 'onclick' => '']) !!}
                                    ...
                                    @can('detailmoney-delete')   
                                    {!! Form::open(['method' => 'post','route' => ['detailmoney.unblock', $module->id ],'style'=>'display:inline', 'onclick' => '']) !!}
                ...
                <div class="tab-pane" id="chaldean" role="tabpanel">
                    <div class="card-block table-border-style">
                        <table class="table table-striped">
                            ...
                            @foreach ($chalddetailed as $key => $modules)
                            <tr>
                                <td>{{ $modules->number }}</td>
                                <td>{!! $modules->description = Str::of($module->description)->limit(90); !!}</td>
                                <td>
                                    <a class="btn btn-info" href="{{ route('detailedmoney.show',$modules->id) }}"><i class="icon-copy ion-eye"></i></a>
                                    @can('chaldname-edit')   
                                    <a class="btn btn-primary" href="{{ route('detailedmoney.edit',$modules->id) }}"><i class="icon-copy ti-pencil-alt"></i></a>
                                    @endcan
                                    @can('chaldname-delete')
                                    {!! Form::open(['method' => 'DELETE','route' => ['detailedmoney.destroy', $modules->id],'style'=>'display:inline', 'onclick' => '']) !!}
