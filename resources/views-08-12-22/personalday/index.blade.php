                ...
                <div class="tab-pane active" id="pythagorean" role="tabpanel">
                    <div class="card-block table-border-style">
                        <table class="table table-striped">
                            ...
                            @foreach ($phythaday as $key => $module)
                            <tr>
                                <td>{{ $module->number }}</td>
                                <td>{!! $module->description !!}</td>
                                <td>
                                    <a class="btn btn-info" href="{{ route('personalday.show',$module->id) }}"><i class="icon-copy ion-eye"></i></a>
                                    @can('persday-edit')
                                    <a class="btn btn-primary" href="{{ route('personalday.edit',$module->id) }}"><i class="icon-copy ti-pencil-alt"></i></a>
                                    @endcan
                                    @can('persday-delete')
                                    {!! Form::open(['method' => 'DELETE','route' => ['personalday.destroy', $module->id],'style'=>'display:inline', 'onclick' => '']) !!}
                ...
                <div class="tab-pane" id="chaldean" role="tabpanel">
                    <div class="card-block table-border-style">
                        <table class="table table-striped">
                            ...
                            @foreach ($chaldday as $key => $modules)
                            <tr>
                                <td>{{ $modules->number }}</td>
                                <td>{!! $modules->description !!}</td>
                                <td>
                                    <a class="btn btn-info" href="{{ route('personalday.show',$modules->id) }}"><i class="icon-copy ion-eye"></i></a>
                                    @can('persday-edit')   
                                    <a class="btn btn-primary" href="{{ route('personalday.edit',$modules->id) }}"><i class="icon-copy ti-pencil-alt"></i></a>
                                    @endcan
                                    @can('persday-delete')
                                    {!! Form::open(['method' => 'DELETE','route' => ['personalday.destroy', $modules->id],'style'=>'display:inline', 'onclick' => '']) !!}
