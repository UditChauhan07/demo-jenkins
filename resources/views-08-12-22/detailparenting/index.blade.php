                ...
                <div class="tab-pane active" id="pythagorean" role="tabpanel">
                    <div class="card-block table-border-style">
                        <table class="table table-striped">
                            ...
                            @foreach ($phythadetail as $key => $module)
                            <tr>
                                <td>{{ $module->number }}</td>
                                <td>{!! $module->description = Str::of($module->description)->limit(90); !!}</td>
                                <td>
                                    <a class="btn btn-info" href="{{ route('detailparenting.show',$module->id) }}"><i class="icon-copy ion-eye"></i></a>
                                    @can('dparenting-edit')
                                    <a class="btn btn-primary" href="{{ route('detailparenting.edit',$module->id) }}"><i class="icon-copy ti-pencil-alt"></i></a>
                                    @endcan
                                    @if($module->is_active == 1)
                                    @can('dparenting-delete')   
                                    {!! Form::open(['method' => 'DELETE','route' => ['detailparenting.destroy', $module->id ],'style'=>'display:inline', 'onclick' => '']) !!}
                                    ...
                                    @can('dparenting-delete')   
                                    {!! Form::open(['method' => 'post','route' => ['detailparent.unblock', $module->id ],'style'=>'display:inline', 'onclick' => '']) !!}
                ...
                <div class="tab-pane" id="chaldean" role="tabpanel">
                    <div class="card-block table-border-style">
                        <table class="table table-striped">
                            ...
                            @foreach ($chalddetail as $key => $modules)
                            <tr>
                                <td>{{ $loop->iteration }}</td>
                                <td>{{ $modules->number }}</td>
                                <td>{!! $modules->description = Str::of($module->description)->limit(90); !!}</td>
                                <td>
                                    <a class="btn btn-info" href="{{ route('detailparenting.show',$modules->id) }}"><i class="icon-copy ion-eye"></i></a>
                                    @can('chaldname-edit')   
                                    <a class="btn btn-primary" href="{{ route('detailparenting.edit',$modules->id) }}"><i class="icon-copy ti-pencil-alt"></i></a>
                                    @endcan
                                    @can('chaldname-delete')
                                    {!! Form::open(['method' => 'DELETE','route' => ['detailparenting.destroy', $modules->id],'style'=>'display:inline', 'onclick' => '']) !!}
